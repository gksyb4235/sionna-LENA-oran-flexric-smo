...
class SionnaRtGui:
    def __init__(self, cfg: GuiConfig):
        ...
        # RET (Remote Electrical Tilt) state relayed from the ns-3/Sionna
        # channel server, keyed by GUI radio device name (e.g. "gnb1"). Kept
        # in sync with SionnaMobilityModel on the ns-3 side; see
        # set_gnb_antenna_state(). Entries missing here default to tilt=0
        # (the array's natural boresight response), matching the "no
        # antenna config = legacy" convention used on the server/ns-3 side.
        self.gnb_antenna_state: dict[str, dict] = {}
    ...
    def set_gnb_antenna_state(
        self, name: str, bearing_deg: float, tilt_deg: float, num_v: int
    ):
        """Apply a RET/sector antenna state relayed from the ns-3/Sionna
        channel server to the named transmitter and refresh the radio map.

        The tilt is applied the same way as the ns-3 channel server
        (ns3sionna_server.py's _tilt_weights/_apply_tx_tilt): as a per-element
        phase taper fed into RadioMapSolver's precoding_vec, not as a
        mechanical rotation of the array. Sionna's tx_array is scene-wide
        (shared by every transmitter, see antenna_array.py), so this mirrors
        the ns-3 side's --gnbVerticalElements being a single value for every
        mmWave gNB in a run: whichever num_v arrives last wins for the whole
        scene. Bearing is still applied per-transmitter via `orientation`,
        since that IS a per-device property in Sionna.
        """
        num_v = max(1, int(num_v))
        if self.scene is not None and self.scene.tx_array.num_ant != num_v:
        ...
        if obj is not None:
            # mi.Point3f (which `orientation`'s setter constructs internally)
            # rejects numpy scalars (np.deg2rad returns np.float64, not a
            # plain float) with "Item assignment failed" -- silently, since
            # this used to be uncaught and would abort the rest of this
            # method (gnb_antenna_state never updated, RadioMap never
            # refreshed). Cast explicitly, same fix as
            # validate_ret_pattern.py/calibrate_radiomap_precoding.py needed.
            yaw = float(np.deg2rad(bearing_deg))
            # electrical tilt (num_v > 1) goes through the precoding vector
            # below; a single element has no array to steer, so fall back to
            # mechanical pitch, exactly like the ns-3 server's
            # _configure_tx_antenna().
        ...
        # Deliberately ignores cfg.radio_map.auto_update (kyunghee.yaml sets it
        # to False to avoid recomputing on every incidental per-frame/position
        # update). A RET change is a rare, explicit xApp-triggered event, not
    ...
    def _tilt_precoding_weights(self, num_v: int, tilt_deg: float) -> np.ndarray:
        """Downtilt co-phasing weights for RadioMapSolver's precoding_vec.

        RadioMapSolver documents h_n = h_n^H @ p (it conjugates the CHANNEL,
        not the weight), which is the opposite convention from
        ns3sionna_server.SionnaEnv._tilt_weights (used with PathSolver.cfr(),
        where h_eff = sum_m conj(w_m) h_m). Empirically calibrated
        (calibrate_radiomap_precoding.py in the scratch validation set): the
        two conventions are exact complex conjugates of each other, so this
        must be the conjugate of the ns-3/server formula for the RadioMap's
        beam to tilt toward positive `tilt_deg` the same way the actual
        ns-3 channel does. Do not "simplify" this to match
        ns3sionna_server.SionnaEnv._tilt_weights -- that was tried and
        verified to steer the beam in the wrong direction here."""
        wavelength = float(self.scene.wavelength.numpy()[0])
        z_m = np.asarray(self.scene.tx_array.positions(wavelength))[2, :].reshape(-1)
        phase = 2.0 * np.pi * (z_m / wavelength) * np.sin(np.deg2rad(tilt_deg))
        return np.exp(1j * phase) / np.sqrt(num_v)

    def _build_precoding_vec(self):
        """Per-transmitter precoding vector reflecting each gNB's relayed
        RET tilt (see set_gnb_antenna_state), for RadioMapSolver(precoding_vec=...).
        Transmitters with no relayed antenna state default to tilt=0 (the
        array's natural boresight response) -- the same "legacy" default used
        throughout this feature when no RET config was ever sent."""
        num_tx = len(self.scene.transmitters)
        num_ant = self.scene.tx_array.num_ant
        if num_tx == 0 or num_ant <= 1:
            return None

        weights = np.zeros((num_tx, num_ant), dtype=np.complex64)
        for i, name in enumerate(self.scene.transmitters.keys()):
            state = self.gnb_antenna_state.get(name)
            tilt_deg = state["tilt_deg"] if state is not None else 0.0
            weights[i, :] = self._tilt_precoding_weights(num_ant, tilt_deg)

        return (mi.TensorXf(np.real(weights)), mi.TensorXf(np.imag(weights)))
    ...
    def compute_radio_map(self) -> rt.RadioMap | None:
        ...
        return solver(
            ...
            precoding_vec=self._build_precoding_vec(),
