...
RicControlMessage::RetControl RicControlMessage::GetRetControl() const
        {
              // RET control message layout (mirrors the Handover Control shapes this
              // decoder already handles; encoder: xapp_ret_ctrl.c):
              //   ranP_List[0]: Target Primary Cell ID, STRUCTURE (same as 8.4.4.1)
              //   ranP_List[1]: RET config (RET_PARAM_CONFIG_STRUCT), STRUCTURE of
              //                 ELEMENTs -- tilt (RET_PARAM_TILT_DEG, mandatory) and
              //                 bearing (RET_PARAM_BEARING_DEG, optional), each an
              //                 ASCII decimal encoded as bit string
              RetControl ret;
              if (m_e2SmRcControlMessageFormat1 == nullptr ||
                  m_e2SmRcControlMessageFormat1->ranP_List.list.count < 2)
                {
                  NS_LOG_ERROR ("RET control message misses the RET config RAN parameter");
                  return ret;
                }

              ret.cellId = GetTargetCell ();

              E2SM_RC_ControlMessage_Format1_Item_t *retStruct =
                  m_e2SmRcControlMessageFormat1->ranP_List.list.array[1];
              if (retStruct->ranParameter_ID != RET_PARAM_CONFIG_STRUCT ||
                  retStruct->ranParameter_valueType.present !=
                      RANParameter_ValueType_PR_ranP_Choice_Structure)
                {
                  NS_LOG_ERROR ("RET config RAN parameter has unexpected id/type");
                  return ret;
                }

              auto *sequence = retStruct->ranParameter_valueType.choice.ranP_Choice_Structure
                                   ->ranParameter_Structure->sequence_of_ranParameters;
              if (sequence == nullptr)
                {
                  NS_LOG_ERROR ("RET config structure is empty");
                  return ret;
                }
              ...
              for (int i = 0; i < sequence->list.count; i++)
                {
                  RANParameter_STRUCTURE_Item_t *item = sequence->list.array[i];
                  if (item->ranParameter_valueType == nullptr ||
                      item->ranParameter_valueType->present !=
                          RANParameter_ValueType_PR_ranP_Choice_ElementFalse)
                    {
                      continue;
                    }
                  BIT_STRING_t *bs = &item->ranParameter_valueType->choice.ranP_Choice_ElementFalse
                                          ->ranParameter_value->choice.valueBitS;
                  std::string value (reinterpret_cast<char *> (bs->buf), bs->size);
                  try
                    {
                      if (item->ranParameter_ID == RET_PARAM_TILT_DEG)
                        {
                          ret.tiltDeg = std::stod (value);
                          hasTilt = true;
                        }
                      else if (item->ranParameter_ID == RET_PARAM_BEARING_DEG)
                        {
                          ret.bearingDeg = std::stod (value);
                          ret.hasBearing = true;
                        }
                    }
                  catch (const std::exception &e)
                    {
                      NS_LOG_ERROR ("Cannot parse RET parameter "
                                    << item->ranParameter_ID << " value '" << value
                                    << "': " << e.what ());
                      return ret;
                    }
                }
