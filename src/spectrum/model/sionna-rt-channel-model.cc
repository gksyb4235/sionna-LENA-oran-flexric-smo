...
SionnaRtChannelModel::LoadScene(const py::module_ rt) const
{
    ...
    if (isXmlFile)
    {
        // Load from XML file. sionna-rt >= 2.0 removed the Scene(filename=...)
        // constructor in favor of routing custom XML files through load_scene()
        // as well (previously only used for built-in named scenes).
        NS_LOG_DEBUG("Loading scene from XML file: " << m_sceneConfigs.sceneName);
        py::object load_scene = rt.attr("load_scene");
        scene = load_scene(py::arg("filename") = m_sceneConfigs.sceneName,
